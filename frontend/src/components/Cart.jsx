...
import { getCart, updateCart } from '../store/actions/productsAction';
// const INITIAL_CART = [
//   {
//     id: 1002,
...
const Cart = () => {
  ...
  const updateQuantity = (id, change, item) => {
  ...
  const handleCheckout = () => dispatch(handleCheckout());
  const resetSystem = () => {
  ...
  const tax = subtotal ? subtotal * 0.08 : 0;
    ...
    return (
      <div className="fixed inset-0 bg-white text-black flex items-center justify-center z-50 overflow-hidden font-sans">
        <div className="relative z-10 text-center space-y-8 p-8 max-w-md w-full">
          <div className="w-24 h-24 mx-auto bg-[#b6ed02] rounded-full flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(182,237,2,0.3)]">
            <CheckCircle className="w-12 h-12 text-black" />
          </div>
          <div>
            <h1 className="text-4xl font-bold uppercase tracking-widest mb-2 text-[#b6ed02]">Order Confirmed</h1>
            <p className="font-mono text-[#8d8d8d] text-sm tracking-wider">ORDER ID: #{Math.floor(Math.random() * 999999)}</p>
          </div>
          <div className="bg-[#F9F8F6] border border-[#8d8d8d]/20 p-6 rounded-sm">
            <div className="flex justify-between text-sm font-mono mb-2">
              <span className="text-[#8d8d8d]">Payment Status</span>
              <span className="text-[#b6ed02] font-bold">Successful</span>
            </div>
          ...
          <button
            onClick={resetSystem}
            className="group relative inline-flex items-center gap-2 px-8 py-3 bg-transparent border border-[#b6ed02] text-[#b6ed02] hover:bg-[#b6ed02] hover:text-black transition-colors uppercase font-bold tracking-widest text-sm rounded-sm"
          >
          ...
          {cart?.length === 0 ? (
            ...
            <div className="space-y-6">
              {cart?.map((item) => (
                ...
                >
                  {/* Advanced Hover Background Blob */}
                    ...
                    <div className="flex-1 flex flex-col justify-between py-1">
                      <div>
                        ...
                        <div className="mt-1 text-xs text-[#8d8d8d] font-mono">
                          <span className="mr-2">Origin: {item.origin}</span>
                        ...
                        <div className="flex items-center gap-6">
                          <div className="flex items-center bg-[#F9F8F6] border border-[#8d8d8d]/20 group-hover:border-[#b6ed02]/50 transition-colors rounded-sm">
                            <button
                              ...
                              className="w-8 h-8 flex items-center justify-center hover:bg-black hover:text-white transition-colors disabled:opacity-20 active:bg-[#b6ed02] active:text-black rounded-l-sm"
                            >
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-[#8d8d8d]/20 p-4 z-40 safe-area-pb">
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            onClick={handleCheckout}
            disabled={cart?.length === 0}
            className="bg-black text-white px-8 py-3 font-bold uppercase text-sm hover:bg-[#b6ed02] hover:text-black transition-colors disabled:opacity-50 rounded-sm"
          >
            Checkout
          </button>
        </div>
